...
int list_near_flights_depature(flights_departure [],near_flight [],int,char [],city_and_time_lenght[]);
int list_near_flights_depature2(flights_arrival  [],near_flight [],int ,char [],city_and_time_lenght[]);
...
int main_flights_depature_2_near_flights(flights_departure [],char [50],city_and_time_lenght[]);
int main_flights_arrival_2_near_flights(flights_arrival [],char [50],city_and_time_lenght[]);
int get_c_and_fl(city_and_time_lenght []);
...
int main(){
    ...
    city_and_time_lenght c_and_fl[32];
    ...
    return_value |= get_c_and_fl(c_and_fl);
    return_value |= main_flights_depature_2_near_flights(flights,time_str,c_and_fl);
    return_value |= main_flights_arrival_2_near_flights(flights2,time_str,c_and_fl);
...
int main_flights_arrival_2_near_flights(flights_arrival flights [], char time_str [50],city_and_time_lenght c_and_fl[])
    {
        FILE *f = NULL;
        ...
        int num_near_flights_arrival = list_near_flights_depature2(flights,near_flights,num_flights,time_str,c_and_fl);
        struct_2_file_and_print2(near_flights,num_near_flights_arrival);
        return 0;
    }
int main_flights_depature_2_near_flights(flights_departure flights [100], char time_str [50],city_and_time_lenght c_and_fl[]) {
    FILE *f = NULL;
    ...
    near_flight near_flights[20];
    int num_near_flights_depature = list_near_flights_depature(flights,near_flights,num_flights,time_str,c_and_fl);
    struct_2_file_and_print(near_flights,num_near_flights_depature);
    return 0;
}
int get_c_and_fl(city_and_time_lenght c_and_fl []) {
    FILE *f = NULL;
    f = fopen("candfl.txt", "r");
    if (!f) {
        printf("cannot open file\n");
        return 1;
    }
    char buf[MAX_WORD_LEN + 1];
    char sv_buf[32][MAX_WORD_LEN + 1];
    int words = 0;
    int is_end =0;
    while (1) {
        int ret = fscanf(f, "%s", buf);
    ...
    for (int i = 0; i < 32;i+=2) {
        c_and_fl[j].city=sv_buf[i];
        c_and_fl[j].flight_time_lenght=sv_buf[i+1];
    }
    char temp[15];
    for (j=0;j<32;j++) {
        for(int i = 0 ; i<15;i++) {
            if(c_and_fl[j].city[i] == '_') {
                for (int z = 0; z<15 ; z++) {
                    temp[z] = c_and_fl[j].city[z];
                }
                temp[i]= '_';
                c_and_fl[j].city = temp;
            }
        }
    }
    return 0;
...
int list_near_flights_depature(flights_departure flights [],near_flight near_flights[],int num_flights,char time[],city_and_time_lenght c_and_fl[]) {
    int count = 0;
    const int real_time = str_time_2_int_time(time);
    int flight_time ;
    int last_i=0;
    for(int i =0; i < num_flights ; i++) {
        flight_time = const_str_time_2_int_time(flights[i].take_of);
        if((flight_time - real_time <=60)&&(flight_time -real_time>=0)) {
            ...
            for(int j = 0 ; j<32;j++) {
                if (c_and_fl[j].city == near_flights[count].destination) {
                    near_flights[count].flight_time_lenght = c_and_fl[j].flight_time_lenght;
                }
            }
            count++;
            last_i=i;
...
int list_near_flights_depature2(flights_arrival flights [],near_flight near_flights[],int num_flights,char time[],city_and_time_lenght c_and_fl[]) {
    int count = 0;
    const int real_time = str_time_2_int_time(time);
    int flight_time ;
    int last_i=0;
